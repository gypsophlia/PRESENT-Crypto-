...
volatile unsigned char tx_fifo[32];
volatile unsigned char rx_fifo[32];

volatile unsigned int tx_fifo_ptA;
...
static void Init_UART()
{
 ...
 GPIO_setOutputLowOnPin(2, (0x0001));
 GPIO_setAsOutputPin(2, (0x0001));

    GPIO_setAsPeripheralModuleFunctionOutputPin(2, (0x0001), (0x02));
    GPIO_setAsPeripheralModuleFunctionInputPin(2, (0x0002), (0x02));
    ...
    param.parity = 0x00;
    param.msborLsbFirst = 0x00;
    param.numberofStopBits = 0x00;
    param.uartMode = (0x0000);
    param.overSampling = 0x01;

    if(0x00 == EUSCI_A_UART_init(0x05C0, &param))
        return;

    EUSCI_A_UART_enable(0x05C0);

    EUSCI_A_UART_clearInterrupt(0x05C0,
                                (0x0001));


    EUSCI_A_UART_enableInterrupt(0x05C0,
                                 (0x0001));
...
unsigned char uart_getc()
{
 ...
 if(rx_fifo_ptA == rx_fifo_ptB)
 {
  rx_flag = 0;
 }
    return c;
}
# 130 "../driverlib/uart_fifo.c"
...
void uart_putc(unsigned char c)
{
 ...
 EUSCI_A_UART_enableInterrupt(0x05C0, (0x0002));
 return;
}
...
void __attribute__((interrupt ((49)))) USCI_A0_ISR(void)
{
 int i;
    switch ((UCA0IV)) {
        case (0x0000): break;
        case (0x0002):
            ...
            i = EUSCI_A_UART_receiveData(0x05C0);


            rx_flag = 1;

   rx_fifo[rx_fifo_ptB] = i & 0xff;
   rx_fifo_ptB++;
   ...
   EUSCI_A_UART_transmitData(0x05C0, tx_fifo[tx_fifo_ptB]);
   ...
   if(tx_fifo_ptB == tx_fifo_ptA)
   {
    EUSCI_A_UART_disableInterrupt(0x05C0, (0x0002));
   }
  break;
        case (0x0006): break;
        case (0x0008): break;
